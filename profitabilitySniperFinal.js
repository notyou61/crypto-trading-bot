...
const EXIT_TARGET = 2.0; // 2x
const PARTIAL_EXIT = 1.3; // 1.3x
const TRADE_SIZE = parseFloat(process.env.TRADE_SIZE || '0.05'); // SOL
...
let simulatedBalance = 0.5; // Paper trading balance (SOL)
let totalProfit = 0;
let tradeCount = [];
...
async function simulateSwap(inputMint, outputMint, amount, price) {
...
async function simulateSell(tokenMint, tokens, price) {
  const solPrice = await getSolPrice();
...
async function evaluateToken(token) {
  ...
  try {
    ...
    if (gain >= ENTRY_TRIGGER && buyers.buyerCount >= 5 && buyers.volume >= 0.3) {
      ...
      const { tokensBought } = await simulateSwap(SOL_MINT, new PublicKey(address), TRADE_SIZE, priceNow);
  ...
  const interval = setInterval(async () => {
    try {
      ...
      if (gain >= EXIT_TARGET && elapsed >= 60000) {
        ...
        await simulateSell(new PublicKey(address), tokensBought, currentPrice);
