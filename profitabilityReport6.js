...
// profitabilityReport6.js - Final Fix for Jupiter Load & Swap Errors

import jupPackage from '@jup-ag/api';
const { Jupiter } = jupPackage;
...
async function executeLiveTrade(targetMint) {
  ...
  if (!routesInfos?.length) {
    console.error('❌ No valid routes found for swap');
    return null;
  }
