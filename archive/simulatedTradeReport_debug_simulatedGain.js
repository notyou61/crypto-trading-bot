// simulatedTradeReport_debug_simulatedGain.js
import fs from 'fs';
...
const ENTRY_TRIGGER = 1.001; // +0.1% gain
const EXIT_TARGET = 3.0;
const PARTIAL_EXIT = 1.5;
const HOLD_LIMIT_MS = 120000;
...
async function simulateTrade(token, solPrice) {
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    try {
      ...
      await delay(8000); // wait to allow price change
      currentPrice = await getTokenPrice(address, Date.now());

      // Simulate price movement if static
      if (currentPrice === entryPrice) {
        console.log(`💡 Simulating gain for ${address} due to static price.`);
        currentPrice = entryPrice * 1.011;
      }
  ...
  while (trades.length < TRADE_LIMIT) {
    try {
      ...
      for (const token of validTokens) {
        ...
        console.log(`📊 ${token.address} entry: ${initial}, current: ${later}`);
        if ((later / initial) >= ENTRY_TRIGGER) {
          ...
          console.log(`⏭ Skipped ${token.address}: Gain ${(later / initial - 1) * 100}% < ${(ENTRY_TRIGGER - 1) * 100}%`);
        }
      }
    } catch (error) {
