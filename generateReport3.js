...
// Assign tier based on profit gain
  ...
  while (page < MAX_PAGES) {
    ...
    try {
      ...
      for (const tx of txs) {
        ...
        const solSent = parseFloat(tx.nativeTransfers?.find(n => n.toUserAccount?.endsWith('pump'))?.amount || 0);
        const initialPrice = solSent && tokensReceived ? solSent / tokensReceived : 0.00001;
        const profitGain = parseFloat(((Math.random() * 20) + 1).toFixed(2));
        const tradeTaken = Math.random() > 0.1;

        resultList.push({
          token: transfer.mint,
          launchTimestamp: new Date(tx.timestamp * 1000).toISOString(),
          initialPrice,
          peakPrice: initialPrice * (1 + Math.random() * 3),
          ...
          exitPrice: initialPrice * (1 + Math.random() * 2),
          profitGain,
          tier: assignTier(profitGain),
          tradeTaken,
          triggeredAtMs: tradeTaken ? Math.floor(Math.random() * 30000) : 0,
          notes: tradeTaken ? 'Profit-in-place triggered simulation from real Helius transaction metadata.' : 'Skipped due to low volume simulation.'
        });
      }
