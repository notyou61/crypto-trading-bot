// simulatedTradeReport.js (debug version)
import fs from 'fs';
...
const ENTRY_TRIGGER = 1.001; // +0.1% gain to trigger more trades for testing
...
async function simulateTrade(token, solPrice) {
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    try {
      ...
      await delay(8000); // Increase delay to allow more movement
