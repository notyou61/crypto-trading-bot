...
import { getBuyerStats, getTokenPrice } from './utils.js';
  ...
  try {
    ...
    for (const token of tokens) {
      ...
      try {
        ...
        if (!buyers || buyers.buyerCount < MIN_BUYERS || buyers.volume < MIN_VOLUME || buyers.volume < 0.1 || buyers.buyerCount < 10) {
...
async function evaluateExits() {
  ...
  for (const [tokenAddress, data] of trackedTokens.entries()) {
    ...
    try {
      console.log(`🔍 Checking exit for ${tokenAddress.slice(0, 8)}...`);
      console.log(`🔍 ${tokenAddress.slice(0, 8)} | Gain: ${(currentPrice / data.entryPrice - 1).toFixed(2)} | Age: ${((now - data.entryTime) / 1000).toFixed(1)}s`);
      const age = now - data.entryTime;
      const currentPrice = await getTokenPrice(tokenAddress, Date.now());
      ...
      const estimatedProfit = data.tokensBought * (currentPrice - data.entryPrice) - TX_FEE;
      if (estimatedProfit <= 0) {
        console.log(`🛑 Skipped exit: Would result in loss (${estimatedProfit.toFixed(6)} SOL)`);
        continue;
      }
      if (shouldSell) {
        data.status = 'sold';
        console.log(`🔍 Selling token ${tokenAddress.slice(0, 8)}...`);
...
async function mainLoop() {
  setInterval(async () => {
    try {
      ...
      console.log(`📈 Trades: ${tradeCount.length} | Net Profit: ${totalProfit.toFixed(4)} SOL | Balance: ${simulatedBalance.toFixed(4)} SOL`);
    } catch (e) {
