...
const assignTier = (gain) => {
  if (gain > 20) return 'Black Hole';
  if (gain > 10) return 'Supermoon';
  if (gain > 5) return 'Moonshot';
  if (gain > 2) return 'Nova';
  return 'Pulse';
};
...
const getBuyerStats = async (tx, startTime) => {
  try {
    ...
    // Count transfers where mint ends with 'pump' as potential buys
