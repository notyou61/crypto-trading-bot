...
// Derive vault address from mint (simplified)
async function getVaultAddress(mintAddress) {
  try {
    // Query Pump.fun program for token creation transaction
    const signatures = await connection.getSignaturesForAddress(new PublicKey(config.PUMP_FUN_PROGRAM), { limit: 1 });
    const tx = await connection.getParsedTransaction(signatures[0].signature);
    const vaultAccount = tx.transaction.message.accountKeys.find(key =>
      key.signer === false && key.writable && key.pubkey.toString() !== mintAddress
    );
    if (!vaultAccount) throw new Error('No vault account found');
    console.log(`INFO: Vault address for ${mintAddress}: ${vaultAccount.pubkey.toString()}`);
    return vaultAccount.pubkey.toString();
  } catch (err) {
    console.warn(`ERROR: Failed to derive vault address for ${mintAddress}: ${err.message}`);
    return config.PUMP_FUN_PROGRAM; // Fallback
  }
}
  ...
  while (Date.now() - start < config.MAX_HOLD_TIME) {
    ...
    if (!tradeExecuted && currentPrice >= initialPrice * 1.10 && tradeLog.buyers10s >= 2) {
      ...
      try {
        ...
        slippageBps: config.SLIPPAGE * 100, // e.g., 3% => 300
        });
        const swapTx = await jupiter.swap(quote);
        const tx = new Transaction().add(...swapTx.instructions);
  ...
  if (tradeExecuted && tradeLog.exitPrice) {
    try {
      const quote = await jupiter.computeRoutes({
        ...
        amount: (config.TRADE_SIZE / tradeLog.entryPrice) * 1_000_000_000,
...
// Fetch buyer stats (from generateReport2.js)
