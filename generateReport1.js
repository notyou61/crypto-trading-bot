...
async function fetchHistoricalData() {
  ...
  while (page < MAX_PAGES) {
    ...
    try {
      ...
      for (const tx of txs) {
        ...
        const profitGain = parseFloat((Math.random() * 30).toFixed(2));
        const tradeTaken = Math.random() > 0.1; // 10% chance to skip trade
        ...
        resultList.push({
          ...
          notes: tradeTaken ? 'Profit-in-place triggered simulation.' : 'Skipped due to low volume simulation.'
        });
      }
  ...
  try {
    ...
    console.log(`✅ Saved ${resultList.length} records to ${OUTPUT_JSON}`);
  } catch (err) {
        ...
        ${tokens.map((token, index) => `
            <div class="section">
                ...
                <table>
                    ...
                    <tr><td>Token</td><td>${token.token}</td></tr>
...
// === Orchestrator ===
(async () => {
  const tokens = await fetchHistoricalData();
